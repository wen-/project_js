    ...
    $.fn.btechcotree = function (options) {
        ...
        function AddNodeToDisplayTree(nodes) {
            ...
            $(nodes).each(function (key, value) {
                ...
                displaytree += "' data-action='nav_items' style='position:relative;top:2px;'></span>";

                if (this.buttoncheck || (this.buttoncheck == null && $settings.show_button_check)) {
                    displaytree += "<input type='checkbox' style='position:relative;top:0px;margin-left:" + item_margin + "px;'></input>";
                    //item_margin += 20;
                }

                if (this.buttonadd || (this.buttonadd == null && $settings.show_button_add)) {
                    displaytree += "<span class='ui-icon " + $settings.class_node_add + "' data-action='add' style='position:relative;top:2px;margin-left:" + item_margin + "px;'></span>";
                    //item_margin += 20;
                }

                if (this.buttonremove || (this.buttonremove == null && $settings.show_button_remove)) {
                    displaytree += "<span class='ui-icon " + $settings.class_node_remove + "' data-action='remove' style='position:relative;top:2px;margin-left:" + item_margin + "px;'></span>";
                    //item_margin += 20;
                }
		...
		$("#" + $settings.containerid).children("ul").find("ul").css({"padding-left":"30px"});
        ...
        function CollapseTree(selectednode) {
            $(selectednode).parent().next("ul").slideToggle("fast");
        }
