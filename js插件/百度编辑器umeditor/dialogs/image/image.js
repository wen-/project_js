...
//                }
    ...
    var Upload = {
        ...
        uploadTpl: '<div class="edui-image-upload%%">' +
			...
			'<input style=\"filter: alpha(opacity=0);\" name="uploadId" class="uploadId_img" />' +
            '</form>' +
        ...
        init: function (editor, $w) {
            ...
            me.submit();
			/*
			//简单跨域判断，不跨域就初始拖拽函数me.drag()
			var current_host = window.location.origin;
			var upload_host = /^(https?:\/\/)?(\w+\.){2,4}\w+(:\d+)?/.exec(um.getOpt('imageUrl'));
			if(!!upload_host && current_host != upload_host[0]){
			}else{
				me.drag();
			}
			*/
                ...
                $('<iframe name="up"  style="display: none"></iframe>').insertBefore(me.dialog).on('load', function(){
					...
					if(!!upload_host && current_host != upload_host[0]){
						function jp(){
							...
							$.ajax({
								 type:'GET',
								 url:'http://192.168.199.198:3000/new_version/headico/upload3',
								 dataType:"jsonp",
								 data:{'uploadId':uuid_img},
					...
					*/
                	//以下是原先不跨域的代码xxx
//					r = this.contentWindow.document.body.innerHTML;
                	r = $(this.contentWindow.document).text();
					if(r == '')return;
					me.uploadComplete(r);
					$(this).unbind('load');
					$(this).remove();
